...
public class InquiryCont {
	...
	public InquiryCont() {
		System.out.println("-----InquiryCont() 객체 생성됨");
	} // 생성자 ends 
	...
	public ModelAndView home(HttpServletRequest req) {
		...
		// 페이징
		List<AdminDTO> alist = adao.paging("inquirylist");
		...
		int totalRowCount = idao.totalRowCount(col, word); // 전체 글 개수
		...
		List<InquiryDTO> list=null;
		  if(totalRowCount>0){
		        list = idao.list(startRow, endRow, col, word);  
		  } else { // 글이 없는 경우
